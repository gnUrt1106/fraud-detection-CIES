...
2. **Dataset phụ ULB**: CIES đã chạy được (`notebooks/05_cies_experiment_ulb.ipynb`, subset `logistic_regression`/`random_forest`/`xgboost` × `class_weighting`/`smote`/`smote_enn`, `feature_level=False`) để so `cies_score` với Sparkov trên cùng tổ hợp — nhưng **chưa có bước benchmark PR-AUC/F1/... riêng cho ULB** (chỉ notebook 03 chạy Sparkov). ULB không cần bước encoding categorical (toàn bộ feature V1..V28 + Amount đã là số), và train subsample cho bootstrap phải giữ nguyên toàn bộ mẫu fraud (chỉ 394 dòng, fraud rate ~0.17%) thay vì random-sample thuần — random-sample có xác suất đáng kể lấy trúng 0 fraud, làm SMOTE crash giữa chừng 1 run (đã tái hiện được lỗi này khi test).
