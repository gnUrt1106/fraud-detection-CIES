...
8. **Vùng tìm Optuna đã nới (2026-09-26)** vì lần tune đầu của thiết kế hiện tại chọn giá trị sát biên cũ: XGBoost Sparkov `n_estimators=550` (90% của 100–600), `learning_rate=0,0137` (9% thang log của 0,01–0,3); RF Sparkov `max_depth=27` (88% của 6–30); LR ULB `C=0,00085` (12% của 1e-4–1e4). Vùng mới, dùng chung cho 2 dataset: `n_estimators`/`iterations` 100–2000, `learning_rate` 0,001–0,3, RF `max_depth` 6–50, LR `C` 1e-6–1e4. Bảng đầy đủ và lý do: [`design_decisions.md`](design_decisions.md) mục 8. (Thiết kế trước cũng có tham số chạm biên — RF `max_depth=30`, CatBoost `depth=11`/12 — nhưng khi đó chưa nới.)
