...
5. **Đợt rà soát toàn bộ mã nguồn — các lỗi đã sửa** (mỗi lỗi có test hồi quy):
   ...
   **Chưa sửa / cần cân nhắc** (không phải bug rõ ràng): (a) `global_mean` trong target encoding tính trên toàn bộ train theo đúng pseudo-code của spec nên hơi rò nhãn fold validation vào prior làm mịn; (b) bootstrap có mẫu trùng nên cùng 1 dòng có thể nằm ở cả fold train lẫn validation của target encoding trong CIES; (c) feature có SHAP = 0 hoà hạng và được xếp theo thứ tự cột nên hơi làm tăng độ ổn định; (d) SMOTE-ENN trên toàn bộ 1.48M dòng ở notebook 03 gần như chắc chắn vượt timeout mặc định 3600s của `run_isolated` (sẽ ghi nhận là lỗi, không mất kết quả khác); (e) SMOTE sinh giá trị phân số cho các cột one-hot.
