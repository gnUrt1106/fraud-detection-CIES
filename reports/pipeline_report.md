...
5. **Đợt rà soát toàn bộ mã nguồn — các lỗi đã sửa** (mỗi lỗi có test hồi quy):
   ...
   **Các điểm còn lại sau rà soát — xử lý như sau:**
   ...
   - *`global_mean` trong target encoding tính trên toàn bộ train* — **cố ý không đổi**: đó là pseudo-code bắt buộc trong spec §3.2, và ảnh hưởng thực tế không đáng kể (1 fold validation chỉ chiếm ~1/5 của 1.48M dòng trong 1 con số trung bình toàn cục). Đổi sẽ buộc encode lại `train_encoded.parquet` và tune lại toàn bộ Optuna để đổi lấy khác biệt không đo được.
   - *SMOTE sinh giá trị phân số cho cột one-hot* — **cố ý không đổi**: spec chốt đúng 5 kỹ thuật với SMOTE chuẩn; chuyển sang SMOTENC sẽ đổi định nghĩa của kỹ thuật đang so sánh.
