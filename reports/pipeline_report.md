...
2. **Dataset phụ ULB**: CIES đã chạy được (`notebooks/05_cies_experiment_ulb.ipynb`, subset `logistic_regression`/`random_forest`/`xgboost` × `class_weighting`/`smote`/`smote_enn`, `feature_level=False`) để so `cies_score` với Sparkov trên cùng tổ hợp — nhưng **chưa có bước benchmark PR-AUC/F1/... riêng cho ULB** (chỉ notebook 03 chạy Sparkov). ULB không cần bước encoding categorical (toàn bộ feature V1..V28 + Amount đã là số). Train subsample cho bootstrap **giữ đúng phương pháp random-sample thuần túy** như Sparkov (không stratify/ép tỷ lệ class — đảm bảo điều kiện xử lý dữ liệu tương đương giữa 2 dataset), nhưng **tăng cỡ mẫu lên ~30,000** (thay vì 10,000 như Sparkov) để bù cho fraud rate thấp hơn của ULB (~0.17% so với ~0.52%) — mục tiêu là số mẫu fraud kỳ vọng tuyệt đối tương đương Sparkov (~52), tránh rủi ro bootstrap resample ra quá ít/0 mẫu fraud làm SMOTE crash (đã tái hiện lỗi này với cỡ mẫu 10,000 cố định trước khi sửa).
