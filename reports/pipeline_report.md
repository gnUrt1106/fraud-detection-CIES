...
1. **Notebook 04/05 chạy đủ 5×5.** Sparkov subsample **phân tầng 100.000 dòng** (`SUBSAMPLE_N`, ≈521 fraud, giữ tỷ lệ 0,52%) vì train đầy đủ 1.48M dòng quá nặng cho 20 run/tổ hợp; mẫu 10.000 cũ chỉ có ~59 fraud (bootstrap còn ~37 fraud khác nhau) nên CIES sẽ đo nhiễu mẫu nhỏ. Kiểm tra độ nhạy (notebook 04 mục 8, `results/cies_sensitivity_subsample.json`, chỉ tổ hợp `xgboost × class_weighting`): CIES = 0,951 / 0,957 / 0,956 / 0,965 ở 10k / 30k / 50k / 100k (52 / 156 / 261 / 521 fraud). Chênh lệch nhỏ, vẫn nhích nhẹ theo cỡ mẫu và nhiễu giữa các run giảm dần; chọn 100k vì ổn định nhất và chạy được. Chưa kiểm tra các tổ hợp khác (SMOTE-ENN, RF, ANN). ULB dùng toàn bộ 227.845 dòng train; SMOTE-ENN ở cỡ này chậm, chạy đủ 25 tổ hợp mất vài giờ.
