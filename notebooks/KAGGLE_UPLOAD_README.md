...
`kaggle_pipeline.ipynb` cần các file đã tiền xử lý trong `data/processed/` (tạo bằng
`notebooks/02_preprocessing.ipynb` ở máy local). Upload chúng thành 1 Kaggle Dataset.
...
|---|---|---|
| `train_encoded.parquet` | ~8 MB | Sparkov (bắt buộc) |
| `test_encoded.parquet` | ~3 MB | Sparkov (bắt buộc) |
| `train_raw.parquet` | ~6 MB | Sparkov (bắt buộc) |
| `test_raw.parquet` | ~3 MB | Sparkov (bắt buộc) |
| `ulb_train.parquet` | ~57 MB | ULB (khi `RUN_ULB = True`) |
| `ulb_test.parquet` | ~15 MB | ULB (khi `RUN_ULB = True`) |

**Nên upload thêm:** 4 tập train Sparkov đã resample trong `data/processed/resampled/`
(`train_encoded_{smote,borderline_smote,adasyn,smote_enn}.parquet`, ~290 MB). Notebook tự dùng lại nếu
dấu vân tay khớp (cùng dữ liệu, seed, version `imbalanced-learn`/`scikit-learn` — notebook ghim đúng
version local) — tiết kiệm ~2,4 giờ SMOTE-ENN mỗi lần benchmark. Không khớp thì notebook tự tính lại,
kết quả vẫn đúng.

**KHÔNG upload** `data/processed/resampled_ulb/`: tên file trùng với bản Sparkov (`train_encoded_*.parquet`)
nên notebook sẽ chép nhầm vào cache Sparkov. ULB resample nhanh (SMOTE-ENN ~1 phút), để Kaggle tự tính.

## Các bước

1. Vào [kaggle.com/datasets](https://www.kaggle.com/datasets) → **New Dataset** (hoặc **New Version** nếu đã có dataset cũ).
2. Kéo thả (hoặc chọn file) các file ở bảng trên từ `data/processed/` (+ 4 file trong `resampled/` nếu upload).
...
7. Chạy từ đầu — cell "Chuẩn bị dữ liệu" tự tìm và symlink các file.
...
`cies_summary_results.json` (và bản `_ulb` của ULB) dở dang từ phiên trước và tiếp tục đúng chỗ dừng.
...
`results/cies_summary_results.json` (và 3 file `_ulb` tương ứng) từ tab Output — chúng đã gồm cả kết quả của repo lẫn của Kaggle —
