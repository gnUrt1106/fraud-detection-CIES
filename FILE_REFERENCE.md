...
| `models/tune.py` | Optuna HPO: TPE + MedianPruner, validation **theo thời gian** (`time_series_folds`: cửa sổ mở rộng, 3 khối validation liên tiếp ở 1/3 cuối tập train — X phải sắp theo thời gian), tối ưu PR-AUC, tune trên dữ liệu **chưa** xử lý imbalance. `tune_all_models` chạy mỗi model trong subprocess, lưu/merge `results/best_params.json` khi model đủ n_trials; hỗ trợ checkpoint SQLite (`checkpoint_dir`) và ngân sách thời gian mềm (`session_budget`) để chia nhiều phiên Kaggle; `load_best_params` để `build_model` tự nạp |
...
| `tests/test_pipeline.py` | 29 test (gồm tiền xử lý Sparkov + chia theo thời gian, fold tune theo thời gian, checkpoint tune, merge kết quả khi nhiều tiến trình ghi song song, file kết quả hỏng không bị ghi đè, độ đồng thuận giữa kỹ thuật): encoding chống rò rỉ (fold prior, index không mặc định, nhóm bootstrap), CatBoost không `cat_features`, 5 kỹ thuật imbalance và one-hot hợp lệ sau resample, công thức CIES (hạng thay vì vị trí cột, hoà hạng), hình dạng SHAP của Tree/ANN, `scale_pos_weight`, ANN (scale, batch 1 mẫu), end-to-end CIES |
