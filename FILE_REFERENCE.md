...
| `imbalance/resamplers.py` | Đúng 5 kỹ thuật: SMOTE, SMOTE-ENN, ADASYN, Borderline-SMOTE (resample) và Class Weighting (trả `class_weights`, không resample). `snap_onehot_groups` / `onehot_groups_from_columns` ép dòng tổng hợp về one-hot hợp lệ khi `SNAP_SYNTHETIC_ONEHOT=True` (mặc định `False`) `apply_imbalance_cached`: benchmark lưu tập train đã resample của mỗi kỹ thuật thành `data/processed/resampled/train_encoded_<kỹ thuật>.parquet` và mọi model dùng chung; metadata chứa dấu vân tay (dữ liệu, seed, SNAP, version imblearn/sklearn), không khớp thì tính lại |
...
| `kaggle_pipeline.ipynb` | Kaggle: tune (50 trial, `MODELS_SCOPE`) → benchmark → CIES cho Sparkov, rồi cho ULB nếu `RUN_ULB` (tham số ULB riêng, cache resample `resampled_ulb/`); với model trong `MODELS_SCOPE` bỏ kết quả clone từ repo và khôi phục tiến độ phiên trước, model ngoài `MODELS_SCOPE` giữ bản trong repo (chạy được nửa local nửa Kaggle), tự resume qua nhiều phiên, ngân sách thời gian chung. Hướng dẫn upload dữ liệu: `KAGGLE_UPLOAD_README.md` |
